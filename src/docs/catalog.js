...
export const UNIFIED_ENDPOINTS = [
  ...
  {
    m: "GET",
    p: "/api/anime/franchise",
    d: "Everything connected to this anime in ONE call: grouped AniList relations (prequel / sequel / sideStory / spinOff / alternative / parent / summary), a flat list with relation tags, a chronological suggestedOrder (prequels → this entry → sequels → sides, every entry deep-linkable by key), a best-effort source-sidebar watchOrder, and AniList recommendations.",
    params: [
      { n: "key", d: "same key formats as /api/anime", ex: "anilist:154587" },
    ],
    try: "/api/anime/franchise?key=frieren",
    tip: "suggestedOrder marks the current entry with current:true — perfect for a watch-page \"Franchise\" tab.",
  },
  ...
  // ---- PLAYBACK ----------------------------------------------------------
  {
    ...
    tip: "verdict field: playable | unverified (probe=0) | no-playable-streams | embed-only (iframe-embeddable, no direct stream) | no-streams. Streams carry kind: direct (real m3u8/mp4 + CORS-ready proxiedUrl for hls.js/<video>) or embed (MegaPlay /videojs/ player page — iframe embedUrl instead). MegaPlay embed URLs are always served under the working /videojs/ prefix and deep-resolved to their real m3u8 (skip ranges + subtitles included).",
  },
  {
    m: "GET",
    p: "/api/watch",
    d: "Stream resolver: returns playable links for one episode. Direct streams carry a proxiedUrl served through the built-in CORS proxies (browsers play them cross-origin out of the box); embed-only servers carry their /videojs/ player page in embedUrl for iframe playback.",
    params: [
      { n: "key", d: "same key formats as /api/anime", ex: "anilist:154587" },
      { n: "ep", d: "episode number, default 1", ex: "1" },
      { n: "type", d: "sub | dub | all", ex: "sub" },
  ...
  },
  {
    m: "GET",
    p: "/api/watch/probe",
    d: "Server-side health check for every stream of an episode: parallel CDN probes report ok + latencyMs per provider. Privacy by design — the response carries NO stream URLs (devtools never sees upstream CDN hosts); match results back onto /api/watch streams by (provider, type). Embed entries are reported without probing.",
    params: [
      { n: "key", d: "same key formats as /api/anime", ex: "anilist:154587" },
      { n: "ep", d: "episode number, default 1", ex: "1" },
      { n: "type", d: "sub | dub | all (default all)", ex: "all" },
    ],
    try: "/api/watch/probe?key=frieren&ep=1",
    tip: "powers Anikuoshi's Auto(best) server mode + latency badges — one probe per episode open, cached.",
  },
  {
    m: "GET",
    p: "/api/download",
  ...
  // ---- META ---------------------------------------------------------------
  {
    m: "GET",
    p: "/api/meta",
    d: "Rich canonical metadata (genres, studios, scores, trailer, external links, next-episode countdown) + cross-provider art (AniList → TMDB → Kitsu) + playback availability for the anime.",
    params: [
      { n: "key", d: "same key formats as /api/anime", ex: "anilist:154587" },
      { n: "season", d: "optional season override for the TMDB per-season poster", ex: "2" },
    ],
  ...
  },
  {
    m: "GET",
    p: "/api/meta/art",
    d: "Cross-provider art fallback chain in ONE call: AniList → TMDB → Kitsu. Returns poster / banner / logo (+ original resolutions), each tagged with the provider that supplied it, plus a per-provider sources[] breakdown.",
    params: [
      { n: "key", d: "same key formats", ex: "anilist:154587" },
      { n: "season", d: "optional season override for the TMDB per-season poster", ex: "2" },
      { n: "verbose", d: "1 = include the per-provider resolution log", ex: "1" },
    ],
    try: "/api/meta/art?key=anilist:154587",
  },
  {
    m: "GET",
    p: "/api/meta/tmdb",
    d: "TMDB provider. Without &ep: show art (poster, backdrop, transparent title logo, per-season posters). With &ep=N: one episode's still + title + air date (absolute-episode → TMDB season mapping handled automatically). Requires TMDB_API_KEY in .env (free at themoviedb.org).",
    params: [
      { n: "key", d: "same key formats", ex: "anilist:154587" },
      { n: "ep", d: "absolute episode number — switches to episode mode", ex: "5" },
      { n: "season", d: "optional season hint override", ex: "2" },
    ],
    try: "/api/meta/tmdb?key=anilist:154587",
  },
  {
    m: "GET",
    p: "/api/meta/kitsu",
    d: "Kitsu provider (keyless). Without &ep: poster + cover art via the MAL↔Kitsu mapping endpoint (title-search fallback). With &ep=N: one episode's thumbnail + title.",
    params: [
      { n: "key", d: "same key formats", ex: "mal:52991" },
      { n: "ep", d: "episode number — switches to episode mode", ex: "5" },
    ],
    try: "/api/meta/kitsu?key=mal:52991",
  },
  {
    m: "GET",
    p: "/api/meta/episode",
    d: "ONE episode's full metadata — title + air date + thumbnail — resolved through the TMDB → Kitsu → AniList streamingEpisodes fallback chain. thumbnailSource tells you which provider won.",
    params: [
      { n: "key", d: "same key formats", ex: "anilist:154587" },
      { n: "ep", d: "absolute episode number", ex: "12" },
      { n: "expectedAired", d: "YYYY-MM-DD sanity check against MAL's own air date (rejects wrong-episode matches)", ex: "2024-06-14" },
      { n: "verbose", d: "1 = include the resolution log", ex: "1" },
    ],
    try: "/api/meta/episode?key=anilist:154587&ep=1",
  },
  {
    m: "GET",
    p: "/api/meta/episodes",
    d: "Every episode with title + air date + thumbnail (same TMDB → Kitsu → AniList chain, concurrency-capped). If the lane episode list undercounts an airing show, the tail is padded from TMDB. &thumbs=0 for a fast thumbnails-free list.",
    params: [
      { n: "key", d: "same key formats", ex: "anilist:154587" },
      { n: "thumbs", d: "0 = skip per-episode imagery (fast)", ex: "0" },
      { n: "verbose", d: "1 = include the resolution log", ex: "1" },
    ],
    try: "/api/meta/episodes?key=anilist:154587&thumbs=1",
  },
  {
    m: "GET",
    p: "/api/mal-list/:user",
    d: "A MyAnimeList user's PUBLIC anime list, normalized (malId, title, status, score, watched/total episodes). Powers the Anikuoshi list import.",
    params: [
      { n: "user", d: "MAL username with a public anime list", ex: "josh" },
    ],
    try: "/api/mal-list/josh",
  },
  {
    m: "GET",
    p: "/api/sync",
    d: "Anikuoshi JSON sync — pull the whole user-data document (progress/lists/history/favorites) for a sync code. Auth: X-Sync-Token header. Requires SYNC_TOKEN in .env. Armor-compatible.",
    params: [],
    try: null,
  },
  {
    m: "PUT",
    p: "/api/sync",
    d: "Push the sync document. Body { rev, doc } with optimistic concurrency — a stale rev answers 409 { conflict, rev, doc } so the client merges and retries. Auth: X-Sync-Token header.",
    params: [],
    try: null,
  },
];
...
export const ENV_VARS = [
  ...
  { n: "TMDB_API_KEY", def: "—", d: "OPTIONAL free TMDB key — activates TMDB art/episode-stills in the meta chain (AniList + Kitsu work without it)." },
  { n: "SYNC_TOKEN", def: "—", d: "OPTIONAL Anikuoshi sync code. When set, /api/sync (GET/PUT/DELETE) stores user-data JSON per code; devices send it as X-Sync-Token." },
  { n: "SYNC_MAX_BYTES", def: "4194304", d: "OPTIONAL max size of one sync document (bytes)." },
  { n: "ARMOR_SECRET", def: "—", d: "OPTIONAL AES-256-GCM response armor. When set, clients sending X-Armor: 1 receive { _armor, iv, ct } envelopes instead of plaintext JSON." },
  { n: "ANIKUOSHI_DIST", def: "auto", d: "OPTIONAL path to the Anikuoshi PWA build. Auto-detected: ./anikuoshi-dist or ./anikuoshi/dist. When present, the API serves the PWA at / with SPA fallback." },
];
...
const FAMILY_META = [
  ...
  { key: "meta", label: "Meta · details", gc: "#f0abfc", note: "Rich metadata: characters, recommendations, seasons, MAL details, external links, cross-provider art (AniList → TMDB → Kitsu), per-episode titles + thumbnails." },

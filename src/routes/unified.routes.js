 ...
 *    ANIME    /anime /anime/episodes /anime/servers /anime/franchise
 *    PLAYBACK /watch /watch/probe /download /chain
...
import { runStreamingChain, malEpisodeIndex, probeStream } from "../core/chain.js";
 ...
 */
/** Shared stream resolution for /api/watch and /api/watch/probe (60s cache). */
async function resolveWatchStreams(anilistId, canonical, ep, type, serverHint) {
  return withCache(`watch:${anilistId}:${ep}:${type}:${serverHint || ""}`, 60, async () => {
    const out = [];
...
router.get("/watch", wrap(async (req, res) => {
  ...
  const streams = await resolveWatchStreams(anilistId, canonical, ep, type, serverHint);
...
/**
 * GET /api/watch/probe?key=...&ep=1 [&type=all]
 * Server-side health check for every resolved stream of an episode.
 *
 * Privacy by design: the response carries NO stream URLs — only the
 * provider name + type + ok/latency, so a devtools Network tab never
 * sees upstream CDN hosts from this endpoint. Clients match the probe
 * results back onto /api/watch streams by (provider, type).
 *
 * Probes run in parallel; each is individually capped by the chain's
 * PROBE_TIMEOUT_MS. Embed pages are reported without probing (they are
 * HTML pages; the embed provider is always "reachable" if listed).
 */
router.get("/watch/probe", wrap(async (req, res) => {
  const key = keyOf(req);
  const ep = parseInt(req.query.ep, 10) || 1;
  const type = ["sub", "dub", "all"].includes(req.query.type) ? req.query.type : "all";
  const { anilistId, canonical } = await canonicalFor(key);

  const streams = await resolveWatchStreams(anilistId, canonical, ep, type, null);
  if (!streams.length) {
    throw new CustomError(`No streams available for episode ${ep} to probe`, 502);
  }

  const probes = await Promise.all(
    streams.map(async (s) => {
      if (s.kind !== "direct" || !s.url) {
        return {
          provider: s.provider, type: s.type, kind: s.kind,
          probed: false, ok: true, latencyMs: null, httpStatus: null,
          detail: "embed — not probed",
        };
      }
      try {
        ...
        return {
          provider: s.provider, type: s.type, kind: s.kind,
          probed: true, ok: Boolean(p.playable), latencyMs: p.latencyMs ?? null,
          httpStatus: p.httpStatus ?? null, detail: p.detail || null,
        };
      } catch (err) {
        return {
          provider: s.provider, type: s.type, kind: s.kind,
          probed: true, ok: false, latencyMs: null, httpStatus: null,
  ...
  res.json({
    ...
    key: keyFor(anilistId), episode: ep, type,
    count: probes.length,
...
/**
 * GET /api/anime/franchise?key=...
 * Everything connected to this anime in ONE call, for the Anikuoshi
 * watch-page "Franchise" tab (and the detail page):
 *   relations       — grouped AniList edges (prequel/sequel/sideStory/
 *                     spinOff/alternative/parent/summary/other), each
 *                     entry with key+format so clients can label
 *                     TV S2 / Movie / OVA / ONA / Special and deep-link
 *   franchise       — the same entries flat with relation tags attached
 *   watchOrder      — best-effort kaze sidebar watch/related order
 *   recommendations — AniList top recommendations
 */
router.get("/anime/franchise", wrap(async (req, res) => {
  const key = keyOf(req);
  const { anilistId, canonical } = await canonicalFor(key);

  const [relations, recs] = await Promise.all([
    anilistRelations(anilistId),
    anilistRecommendations(anilistId, 10),
  ]);

  // Watch-order / related sidebar — best effort, cached, never fatal.
  let watchOrder = null;
  try {
    const kaze = getLane("kaze");
    const match = await matchSlug(kaze, canonical);
    if (match?.listingId) {
      const wo = await withCache(`wo:${match.listingId}`, 21600, () => kaze.watchOrder(match.listingId));
      const raw = wo?.data?.related || wo?.related || [];
      const related = raw
        .filter((r) => r?.slug && r.slug !== match.listingId)
        .map(({ url, ...rest }) => rest) // drop internal page URLs — slug is enough
        .slice(0, 20);
      if (related.length) watchOrder = { slug: match.listingId, count: related.length, related };
    }
  } catch (err) {
    console.error("[APIKUOSHI][franchise] watch-order skipped:", err.message);
  }

  const groups = relations || {};
  const flat = Object.entries(groups).flatMap(([relation, list]) =>
    (list || []).map((e) => ({ ...e, relation }))
  );

  // Reliable chronological watch order computed FROM the relations graph:
  // prequels (oldest first) -> this entry -> sequels (oldest first) ->
  // side stories & spin-offs. Every entry carries a real key so clients
  // can deep-link straight into playback.
  const orderRole = (role) => (role === "prequel" ? 0 : role === "current" ? 1 : role === "sequel" ? 2 : 3);
  const self = {
    key: keyFor(anilistId),
    anilistId,
    malId: canonical.malId ?? null,
  ...
  const sideEntries = [...(groups.sideStory || []), ...(groups.spinOff || [])]
    .map((e) => ({ ...e, orderRole: "side" }));
  const suggestedOrder = [
    ...(groups.prequel || []).map((e) => ({ ...e, orderRole: "prequel" })),
    self,
    ...(groups.sequel || []).map((e) => ({ ...e, orderRole: "sequel" })),
    ...sideEntries,
  ].sort((a, b) => orderRole(a.orderRole) - orderRole(b.orderRole) || (a.year ?? 9999) - (b.year ?? 9999));
...
// ================================================================ MAL LIST
// A MAL user's PUBLIC anime list, normalized for the Anikuoshi import flow.

router.get("/mal-list/:user", wrap(async (req, res) => {
  const user = String(req.params.user || "").trim();
  if (!user || /[^A-Za-z0-9_-]/.test(user)) throw new CustomError("Invalid MAL username", 400);

  const url = `https://myanimelist.net/animelist/${encodeURIComponent(user)}/load.json?status=7&offset=0`;
  const r = await axios.get(url, {
    timeout: 20000,
    headers: { "User-Agent": PROXY_UA, Accept: "application/json, text/plain, */*" },
  });
  const arr = Array.isArray(r.data) ? r.data : Array.isArray(r.data?.data) ? r.data.data : [];

  const statusName = (s) => {
    if (typeof s === "number") {
      return { 1: "watching", 2: "completed", 3: "on_hold", 4: "dropped", 6: "plan_to_watch", 7: "plan_to_watch" }[s] || "plan_to_watch";
    }
    const v = String(s || "").toLowerCase();
    if (v.includes("watch")) return "watching";
  ...
  const entries = arr
    .map((e) => ({
      malId: e.anime_id ?? e.id ?? null,
      title: e.anime_title ?? e.title ?? e.anime_title_eng ?? null,
      image: e.anime_image_path ?? e.image_url ?? null,
      status: statusName(e.status),
      score: typeof e.score === "number" && e.score > 0 ? e.score : null,
      watchedEpisodes: Number(e.num_watched_episodes ?? e.watched ?? 0) || 0,
      totalEpisodes: Number(e.anime_num_episodes ?? e.total ?? 0) || null,
    }))
    .filter((e) => e.malId && e.title);

  if (!entries.length) {
    throw new CustomError(`No public list entries for MAL user "${user}" (list may be private or empty)`, 404);
  }
  res.json({ success: true, api: "APIKuoshi", user, count: entries.length, entries });
}));

...
const extractStreamInfo = async (linkId, watchSlug = null) => {
  try {
    ...
    // NOTE: anikoto's "stream url" is usually an EMBED page (megaplay/vidplay), not a raw media file.
    // Classify precisely so clients can choose <iframe> vs <video> playback.
    let streamType = null;
    if (streamUrl) {
      if (/\.m3u8(\?|$)/.test(streamUrl)) streamType = "hls";
      else if (/\.mp4(\?|$)/.test(streamUrl)) streamType = "mp4";
      else if (/megaplay\.buzz|vidplay\.site|vid-tube\.site|vidtube\.site/i.test(streamUrl)) streamType = "embed";
 ...
 *
 * NOTE: The real mapper endpoint is https://mapper.nekostream.site/api/mal/
 * (as used by anikoto's own mapper.js). The legacy /ajax/mapper path on the
 * mirror is kept as a secondary attempt. All failures degrade to [].
 *
...
const extractMapperServers = async (malId, slug, timestamp) => {
  try {
    ...
    const seg = `${encodeURIComponent(malId)}/${encodeURIComponent(slug)}/${encodeURIComponent(timestamp)}`;
    const candidateFetches = [
      // 1) Real mapper API (used by anikoto's own mapper.js)
      async () => {
        const res = await axios.get(`https://mapper.nekostream.site/api/mal/${seg}`, {
          headers: { ...headers, Referer: "https://megaplay.buzz/" },
          timeout: 8000, validateStatus: () => true,
        });
        return typeof res.data === "object" ? res.data : null;
      },
      // 2) Legacy mirror path (kept for older deployments)
      async () => {
        const { data: raw } = await fetchWithMirror(`/ajax/mapper/${seg}`);
        return typeof raw === "string" ? (() => { try { return JSON.parse(raw); } catch { return null; } })() : raw;
      },
    ];

    let data = null;
    for (const fetcher of candidateFetches) {
      try {

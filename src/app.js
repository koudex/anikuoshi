...
import { armorMiddleware } from "./core/armor.js";
import systemRoutes from "./routes/system.routes.js";
import unifiedRoutes from "./routes/unified.routes.js";
import syncModule from "./routes/sync.routes.js";
import appRoutes, { findAnikuoshiDist } from "./routes/app.routes.js";
...
// JSON bodies — /api/sync carries full user-data documents, so it parses
// later with its own (larger) limit. Everything else stays strict.
app.use((req, res, next) => {
  if (req.path.startsWith("/api/sync")) return next();
  express.json({ limit: "100kb" })(req, res, next);
});

// Optional AES-GCM response armor for clients that ask with X-Armor: 1
...
// landing page — JSON unless the Anikuoshi PWA build is present
app.get("/", (req, res, next) => {
  if (findAnikuoshiDist()) return next(); // app.routes serves the PWA shell
  res.json({
    success: true,
    api: "APIKuoshi",
    version,
...
// 1.5 sync (own JSON limit — full user-data documents)
app.use("/api/sync", express.json({ limit: syncModule.jsonLimit }), syncModule.router);

// 2. the API surface
app.use("/api", unifiedRoutes);

// 2.5 Anikuoshi PWA build (static + SPA fallback) — no-op without the build
app.use(appRoutes);

...
### Meta
`/api/meta?key=` (includes a cross-provider `art` block) · `/api/meta/characters?key=&limit=` · `/api/meta/recommendations?key=&limit=` · `/api/meta/season?season=&year=` · `/api/meta/mal?key=&episodes=1` · `/api/meta/external?key=` · `/api/meta/trending` · `/api/meta/art?key=` · `/api/meta/tmdb?key=&ep=` · `/api/meta/kitsu?key=&ep=` · `/api/meta/episode?key=&ep=` · `/api/meta/episodes?key=&thumbs=`

**Cross-provider art & episode imagery** (design ported from
[AniVault-Scraper](https://github.com/SH0MIK/AniVault-Scraper)):

| Provider | Needs a key? | Provides |
|---|---|---|
| **AniList** GraphQL | no (keyless) | poster + banner, MAL↔AniList identity mapping, season lists, streaming-episode titles/thumbnails (fallback) |
| **TMDB** REST | free key (`TMDB_API_KEY` in `.env`) | poster / backdrop / **transparent title logo**, per-season posters, episode stills + real titles + air dates, absolute-episode → season mapping |
| **Kitsu** JSON:API | no (keyless) | poster + cover, MAL↔Kitsu ID mapping, episode thumbnails/titles |
| **MyAnimeList** (direct scrape) | no (keyless) | details, characters, episode lists, external/streaming links (via `/api/meta/mal`, `/api/meta/external`) |

Art resolves through a fallback chain — **AniList → TMDB → Kitsu** — and
per-episode imagery through **TMDB → Kitsu → AniList streamingEpisodes**;
every response tags each image with the provider that supplied it
(`posterSource`, `thumbnailSource`, `art.sources[]`). TMDB is optional:
without a key the chain degrades gracefully (AniList + Kitsu still answer)
and `/api/meta/tmdb` returns an actionable 501.
...
| `TMDB_API_KEY` | — | **Optional** free TMDB key — activates TMDB art/episode stills in the meta chain |
| `SYNC_TOKEN` | — | **Optional** Anikuoshi sync code — enables `/api/sync` (devices send it as `X-Sync-Token`) |
| `SYNC_MAX_BYTES` | `4194304` | Max size of one sync document |
| `ARMOR_SECRET` | — | **Optional** AES-256-GCM response armor for clients sending `X-Armor: 1` |
| `ANIKUOSHI_DIST` | auto | Path to the Anikuoshi PWA build (auto-detects `./anikuoshi-dist`, `./anikuoshi/dist`) |

---

## Anikuoshi PWA

APIKuoshi ships as the backend for **Anikuoshi** — a no-login anime streaming PWA
(build included as `anikuoshi-dist/`, source zip on the download page). When a
built PWA is present the API serves it at `/` automatically:

- Drop the PWA's `dist/` folder in as `anikuoshi-dist/`, or set `ANIKUOSHI_DIST`.
- The JSON landing page only shows when no PWA build is installed.
- **Sync**: set `SYNC_TOKEN=KUOSHI-XXXXXX` and devices holding that code store
  progress/lists/history/favorites as one JSON document at `/api/sync`
  (`GET` pull, `PUT` push with rev-conflict merges, `DELETE` wipe).
- **Armor**: set `ARMOR_SECRET` and armored clients (the PWA with the same
  secret in Settings) receive AES-256-GCM-encrypted JSON envelopes — the
  Network tab shows ciphertext, not titles or stream URLs. Plain clients and
  curl are unaffected.
- **MAL import**: `/api/mal-list/:user` normalizes a public MyAnimeList list
  for the PWA's import flow.
- Stream/proxy infrastructure means upstream CDNs never appear in client
  devtools — everything rides your origin.
